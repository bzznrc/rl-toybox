...
<p align="center">
  <img src="media/clip-placeholder.svg" alt="Reserved clip slot" width="32%" />
  <img src="media/clip-placeholder.svg" alt="Reserved clip slot" width="32%" />
  <img src="media/clip-placeholder.svg" alt="Reserved clip slot" width="32%" />
</p>
...
`play_ai` loads `best` by default, so `--model best` is shown only to make the artifact choice explicit. `osero` board size is selected through `OSERO_BOARD_SIZE`; `6x6` is the default.
...
| `jump` | On-policy traversal game | actor-critic | Compact side-view micro-platformer built around short procedural runs, timing windows, and simple left/right/jump control | [games/jump/README.md](games/jump/README.md) |
...
| `osero` | Planning and self-play capstone | search + self-play | Compact Osero/Reversi implementation using MCTS, self-play, and a small policy/value network | [games/osero/README.md](games/osero/README.md) |
| `kick` | Multi-agent football / CTDE showcase | multi-agent actor-critic / CTDE | Shared-policy top-down `7v7` football environment with centralized-critic PPO training | [games/kick/README.md](games/kick/README.md) |
...
- `jump` -> `ppo`, `obs=24`, `act=4`, actor `24 -> 32 -> 32 -> 4`, critic `24 -> 32 -> 32 -> 1`
