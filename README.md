...
- `core/search_play/` contains the compact MCTS, policy/value, and self-play stack used by `four`.
...
<p align="center">
  ...
  <img src="media/four-demo.gif" alt="Four demo clip" width="32%" />
...
Training prints compact single-line progress records. `Ep:` lines show environment performance: episode length, reward, rolling reward, best reward for the level, success, average success, and optional reward components. Actor-critic runs also print `Up:` once per optimizer update. PPO-style updates use `Pi` for policy loss, `V` for value loss, `EV` for critic explained variance, `Ent` for entropy, and `KL` for approximate KL. SAC-style updates use `Pi`, `Q`, `Ent`, and `Alpha`.
...
| `four` | Planning + self-play capstone | search + self-play | Fixed Connect Four implementation using MCTS, self-play, legal-column masking, and a small policy/value network | [games/four/README.md](games/four/README.md) |
...
- `four` -> `search_play`, fixed `7x6`, `obs=42`, `act=7`, policy/value net `42 -> 64 -> 64 -> (7 + 1)`
