...
Per-game snapshots or scaffold notes live in `games/<game>/README.md`.
...
- [7) Legacy and Paused Areas](#7-legacy-and-paused-areas)
...
- `core/value_discrete/`
  ...
  - home for value-based helpers such as Double DQN, PER, Dueling, and action masking support
- `core/actor_critic/`
...
## 7) Legacy and Paused Areas

- `kick` stays in-tree and runnable, but should be labeled paused or experimental in repo-facing docs.
- `vroom` now sits on the actor-critic / continuous-control branch with SAC-oriented defaults.
...
- [ ] `kick` remains clearly marked as paused when touched.
