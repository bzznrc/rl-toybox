...
TORSO_BORDER_INSET_PX = float(CELL_INSET)
LEG_INNER_WIDTH_PX = max(2.0, float(TILE_SIZE) * 0.20)
LEG_OUTER_WIDTH_PX = float(LEG_INNER_WIDTH_PX + 2.0 * float(CELL_INSET))
FOOT_INNER_WIDTH_PX = max(2.0, float(TILE_SIZE) * 0.22)
FOOT_OUTER_WIDTH_PX = float(FOOT_INNER_WIDTH_PX + 2.0 * float(CELL_INSET))

DISTANCE_MARKER_SPACING_METERS = 1.0
DISTANCE_MARKER_MAJOR_EVERY = 10
DISTANCE_MARKER_HEIGHT_FRACTION = 0.25
DISTANCE_MARKER_INNER_SIZE_PX = max(2.0, float(TILE_SIZE - 2 * CELL_INSET))
DISTANCE_MARKER_OUTLINE_INSET_PX = float(CELL_INSET)
