...
class PPOAlgorithm(Algorithm):
    ...
    def __init__(self, config: PPOConfig):
        ...
        if self._is_discrete:
            self._action_low = np.asarray([-1.0], dtype=np.float32)
            self._action_high = np.asarray([1.0], dtype=np.float32)
        else:
            self._action_low = np.asarray(config.action_low, dtype=np.float32).reshape(-1)
            self._action_high = np.asarray(config.action_high, dtype=np.float32).reshape(-1)
            if self._action_low.size == 1:
                self._action_low = np.full((int(config.action_dim),), float(self._action_low.item()), dtype=np.float32)
            if self._action_high.size == 1:
                self._action_high = np.full((int(config.action_dim),), float(self._action_high.item()), dtype=np.float32)
            if self._action_low.shape != (int(config.action_dim),) or self._action_high.shape != (int(config.action_dim),):
                raise ValueError("PPO continuous action bounds must match action_dim.")
            if np.any(self._action_low >= self._action_high):
    ...
    def _clip_continuous_actions(self, action_batch: np.ndarray) -> np.ndarray:
        clipped = np.clip(
            np.asarray(action_batch, dtype=np.float32),
            self._action_low.reshape(1, -1),
            self._action_high.reshape(1, -1),
        )
